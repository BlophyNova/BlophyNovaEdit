...
namespace Controller
{
    public class HoldController : NoteController
    {
        ...
        public override void NoteHoldArise()
        {
            ...
            {
                if (Time.time - checkTime > ValueManager.Instance.holdLeaveScreenTime &&
                    !isMissed) //如果当前时间距离手指在我这里判定的最后一帧已经超过预先设置的时间了并且没有Miss
                {
                    isMissed = true; //这个条件下肯定已经Miss了，设置状态
                    HoldMiss(); //调用Miss函数
                }
                else if (Time.time - checkTime <= ValueManager.Instance.holdHitEffectCdTime && !isMissed &&
                         reJudge) //如果当前时间距离手指在我这里判定的最后一帧没有超过预先设置的时间并且没有Miss和可以进行重判了
                {
                    //checkTime = Time.time;
                    //没有Miss
                    //打击特效
            ...
            if (!(ProgressManager.Instance.CurrentTime >= thisNote.hitTime + JudgeManager.Bad) || isJudged ||
                isMissed) //如果当前时间已经超过了打击时间+bad时间（也就是一开始就没有按下手指）并且没有判定过并且没有Miss
            {
                return;
            }
        ...
        public override void PassHitTime(double currentTime)
        {
            ...
            transform.localPosition =
                new Vector2(transform.localPosition.x, -noteCanvas.localPosition.y); //将位置保留到判定线的位置
