...
namespace Controller
{
    public class PointController : NoteController
    {
        ...
        public override void NoteHoldArise()
        {
            ...
            float percent = ((float)ProgressManager.Instance.CurrentTime - ariseTime) /
                            (thisNote.hitTime - ariseTime); //计算现在是出现时间到打击时间为百分之多少的百分比
            if (percent > 1) //如果大于1
            {
                return; //返回
            }
            ...
            for (int i = 0; i < JourneyLength; i++) //循环--遍历所有角，计算位置
            {
                ...
                moveEdgeCorner[i].localPosition =
                    (destinations[i] - origins[i]) * percent + origins[i]; //（终点-起点）*百分比+起点
            }
        }
        ...
        private void SetTextureLocalScale()
        {
            foreach (SpriteRenderer item in allHorizontal) //水平
            {
                item.transform.localScale =
                    new Vector2(.5f + ValueManager.Instance.boxFineness / decideLineController.box.currentScaleX,
                        decideLineController.box.spriteRenderers[0].transform.localScale.y); //计算XY并赋值
            }

            foreach (SpriteRenderer item in allVertical) //垂直
            {
                item.transform.localScale =
                    new Vector2(.45f - ValueManager.Instance.boxFineness / decideLineController.box.currentScaleY,
                        decideLineController.box.spriteRenderers[2].transform.localScale.y); //计算XY并赋值
            }
        }
