...
namespace Form.NoteEdit
{
    ...
    public partial class NoteEdit
    {
        public void AddNote2ChartData(Note note, int boxID, int lineID)
        {
            ...
            int index = Algorithm.BinarySearch(notes,
                m => m.hitTime < BPMManager.Instance.GetSecondsTimeByBeats(note.HitBeats.ThisStartBPM), false);
            Data.ChartData.Note newNote = new(note);

            newNote.hitFloorPosition =
                (float)Math.Round(ChartData.boxes[boxID].lines[lineID].far.Evaluate(newNote.hitTime), 3);
            ...
            GlobalData.Refresh<IRefreshPlayer>(interfaceMethod => interfaceMethod.RefreshPlayer(-1, -1),
                new List<Type> { typeof(LineNoteController) }); //定向刷新，终于不再是以前的暴力刷新所有东西了
        }
