...
namespace Form.EventEdit
{
    public class EventVerticalLine : MonoBehaviour
    {
        ...
        public List<Data.ChartEdit.Event> events
        {
            get
            {
                var boxEvents = GlobalData.Instance.chartEditData.boxes[currentBoxID].boxEvents;
        ...
        private void Update()
        {
            displayCurrentEventValue.text = $"{(int)func(CalculateCurrentValue(events,BPMManager.Instance.GetCurrentBeatsWithSecondsTime((float)ProgressManager.Instance.CurrentTime),ref defaultValue))}";
        }
        /// <summary>
        ///     计算当前数值
        /// </summary>
        ...
        /// <returns></returns>
        private static float CalculateCurrentValue(List<Data.ChartEdit.Event> events, float currentBeats, ref float defaultValue)
        {
            ...
            static bool IsCurrentEvent(Data.ChartEdit.Event m, ref float currentTime)
            {
